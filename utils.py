...
def process_batches(boxes_list, labels_list, detection_preds, classification_preds, confidence_preds, device, new_w, new_h, epoch, n, images):
    for boxes, labels, det_preds, class_preds, conf_preds, image in zip(boxes_list, labels_list, detection_preds, classification_preds, confidence_preds, images):
        boxes, det_preds, labels, class_preds, conf_preds, image = boxes.to(device), det_preds.to(device), labels.to(device), class_preds.to(device), conf_preds.to(device), image.to(device)
        #save_images(boxes, det_preds, new_w, new_h, 0, images)
        #n += 1
        yield boxes, labels, det_preds, class_preds, conf_preds, image
